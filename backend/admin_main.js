...
const sessionTimeout = 60_000 * 60 * 3;
...
const DOMAIN = "https://career-admin.alsalamah506.sa/api";
const db = {
...
app.post(
  ...
  async function (req, res) {
    ...
    const [querySuccess, queryResult] = await SQL.query(
...
app.get(
  "/api/application_files/:applicationKey/:fileName",
  authenticateClient,
  async function (req, res) {
...
app.get(
  "/api/application_files/:fileName",
  authenticateClient,
  async function (req, res) {
...
async function getJobApplications(res, SQL, force) {
  if (force || Date.now() - applications.lastUpdate > 1000 * 60) {
    const queryStr =
      "SELECT ja.`id` as appId, ja.`start_date`, ja.`closed`, j.`title` FROM `" +
      db.job_applications +
      "` ja LEFT JOIN `" +
      db.jobs +
      "` j ON(j.`id` = ja.`job_id`) ORDER BY ja.`start_date` DESC;";
...
async function getJobSubmissions(res, SQL, applicationId) {
  ...
  if (!applicationId) {
    ...
    jobApplication = applications.jobApplications.find(
...
function chooseFilterFunction(searchText) {
  ...
  if (!isDigit(searchText)) {
    // if search text is not only numbers, search by name
    filterFunc = filterByName;
  } else {
    // otherwise
    if (searchText.length === 10) {
      // if it's 10 digits
      if (searchText.startsWith("1") || searchText.startsWith("2")) {
        // if it starts with 1 or 2, then search by id_number
      ...
      // otherwise, search by submission id
...
app.get(
  "/api/submissions/:appId/search/:searchText",
  rateLimiter.adminGlobalLimiter,
  authenticateClient,
  async function (req, res) {
    const appId = isDigit(req.params.appId) && parseInt(req.params.appId);
    const searchText =
      (req.params.searchText.length > 0 && decodeURI(req.params.searchText)) ||
      false;
    if (!appId || !searchText) {
      res.status(400).json({
        success: false,
...
app.get(
  ...
  async function (req, res) {
    const appId =
      (req.query.appId ? true : false) &&
      isDigit(req.query.appId) &&
      parseInt(req.query.appId);
    const categoryId =
      (req.query.category ? true : false) &&
      isDigit(req.query.category) &&
      parseInt(req.query.category);
    const page =
      (req.query.page ? true : false) &&
      isDigit(req.query.page) &&
      parseInt(req.query.page);
    const searchText =
      ((req.query.s ? true : false) && decodeURI(req.query.s)) || false;
    if (
      appId
        ? appId < 1
        : false || // if appId is provided and its value less than 1
          categoryId
        ? categoryId < 0 || categoryId > 3
        : false || // or if category is provided and its value less than 0 or greater than 3
          page
        ? page < 1
        : false // or if page is provided and its value less than 1
    ) {
      // then
      res.status(400).json({
        success: false,
    ...
    for (const [category, value] of Object.entries(submissions)) {
      ...
      endSubmission = Math.min(
        startSubmission + submissionsPageLimit,
        value.count
      );
...
app.get(
  "/api/submissions/:appId/:categoryId/pages/:page",
  rateLimiter.adminGlobalLimiter,
  authenticateClient,
  async function (req, res) {
    const applicationId =
      isDigit(req.params.appId) && parseInt(req.params.appId);
    const categoryId =
      isDigit(req.params.categoryId) && parseInt(req.params.categoryId);
    ...
    SQL.disconnect();
    const jobApplication = applications.jobApplications.find(
      (value) => value.id === applicationId
    );
    if (!jobApplication) {
      res.status(404).json({
    ...
    */
    ...
    if (searchText) {
      const filterFunc = chooseFilterFunction(searchText);
      categorySubmissions.rows = categorySubmissions.rows.filter(
        filterFunc,
        searchText
      );
    ...
    const totalPages = Math.ceil(
      categorySubmissions.count / submissionsPageLimit
    );
    if (page > totalPages) {
      res.status(404).json({
        success: false,
    ...
    const endSubmission = Math.min(
      startSubmission + submissionsPageLimit,
      categorySubmissions.count
    );
...
app.get(
  "/api/submissions/:id",
  rateLimiter.adminGlobalLimiter,
  /* authenticateClient, */ async function (req, res) {
    ...
    if (!submission) {
      ...
      if (!SQL) return;
      const qStr =
        "SELECT js.*, ja.`job_title` FROM `" +
        db.job_submissions +
        "` js " +
        "LEFT JOIN `" +
        db.job_applications +
        "` ja ON(ja.`id` = js.`application_id`) " +
...
app.patch(
  ...
  async function (req, res) {
    ...
    const submissionNewStatus =
      isDigit(req.body.status) && parseInt(req.body.status);

    if (
      submissionId === false ||
      submissionNewStatus === false ||
      submissionId < 1 ||
      submissionNewStatus < 0 ||
      submissionNewStatus > 3 ||
      !Number.isInteger(submissionId) ||
      !Number.isInteger(submissionNewStatus)
    ) {
      res.status(400).json({
        success: false,
        code: 400,
    ...
    const [getStatusSuccess, getStatusQuery] = await SQL.query(
      "SELECT `status` FROM `" +
        db.job_submissions +
        "` WHERE `id` = ? LIMIT 1;",
      [submissionId]
    ...
    if (getStatusQuery[0].status === submissionNewStatus) {
      ...
      res.status(403).json({
        ...
        code: 403,
        message:
          "You can't change the submission status after it has been accepted.",
      });
      return;
    }
    ...
    if (submissionNewStatus === 1) {
      const interviewDate =
        isDigit(req.body.interviewDate) && parseInt(req.body.interviewDate);
      const interviewLocationId =
        isDigit(req.body.interviewLocation) &&
        parseInt(req.body.interviewLocation);
      ...
      if (
        ...
        isNaN(interviewLocationId) ||
        !isFinite(interviewLocationId)
      ) {
        SQL.disconnect();
        res.status(400).json({
          success: false,
      ...
      const [locationSuccess, locationQuery] = await SQL.query(
        "SELECT * FROM `" +
          db.interview_locations +
          "` WHERE `id` = ? LIMIT 1;",
        [interviewLocationId]
      );
      if (!locationSuccess) {
      ...
      if (!foundSubmission) {
        const qStr =
          "SELECT js.`id`, js.`name`, js.`phone`, ja.`job_title` as jobName FROM `" +
          db.job_submissions +
          "` js " +
          "LEFT JOIN `" +
          db.job_applications +
        ...
        const [querySuccess, queryResult] = await SQL.query(qStr, [
          submissionId,
        ]);
        if (!querySuccess) {
          console.error("API: PATCH@'/api/submissions'\n", queryResult);
      ...
      if (!process.env.DEVELOPMENT_ENVIRONMENT) {
        ...
        const formatTime = interviewRealDate
        ...
        const body = {
          channel: "whatsapp",
          type: "buttonTemplate",
          recipient: foundSubmission.phone,
          templateId: process.env.WHATSAPP_INTERVIEW_TEMPLATE,
          templateArgs: [
            ...
            `=${encodeURIComponent(locationQuery[0].url)}`,
          ],
          templateLanguage: process.env.WHATSAPP_INTERVIEW_LANGUAGE_CODE,
        };
        let result;
        try {
          result = await node_fetch("https://api.mottasl.com/v2/message", {
            method: "POST",
          ...
          console.error("API Error: PATCH@'/api/submissions'\n", error);
          const message =
            (error.message && `${error.message}`) ||
            "Couldn't send message to whatsapp.";
          res.status(504).json({
            success: false,
            code: 504,
            message: message,
      ...
      const logAcceptanceQueryString =
        "INSERT INTO `" +
        db.accepted_submissions +
        "` (`submission_id`, `interview_date`, `interview_location_name`, `interview_location_url`, `interview_notes`, `accepted_by`) VALUES(?,?,?,?,?,?);";
      const [logAcceptanceSuccess, logAcceptanceQuery] = await SQL.query(
        logAcceptanceQueryString,
        [
          submissionId,
          interviewRealDate,
          locationQuery[0].name,
    ...
    const [updateStatusSuccess, updateStatusQuery] = await SQL.query(
      "UPDATE `" + db.job_submissions + "` SET `status` = ? WHERE `id` = ?",
      [submissionNewStatus, submissionId]
    );
    SQL.disconnect();
...
app.get(
  ...
  async function (req, res) {
    ...
    const [settingsSuccess, settingsQuery] = await SQL.query(
      "SELECT * FROM `" + db.interview_locations + "` ORDER BY `id` ASC;"
    );
    SQL.disconnect();
    if (!settingsSuccess) {
      res.status(500).json({
...
app.get(
  ...
  async function (req, res) {
    ...
    const [locationsSuccess, locationsQuery] = await SQL.query(
      "SELECT * FROM `" + db.interview_locations + "` ORDER BY `id` ASC;"
    );
    SQL.disconnect();
    if (!locationsSuccess) {
...
app.post(
  ...
  async function (req, res) {
    ...
    const [insertSuccess, insertQuery] = await SQL.query(
      "INSERT INTO `" +
        db.interview_locations +
        "` (`name`, `url`) VALUES (?, ?)",
      [name, url]
    ...
    const [selectSuccess, selectQuery] = await SQL.query(
      "SELECT * FROM `" + db.interview_locations + "` ORDER BY `id` ASC;"
    );
    SQL.disconnect();
    if (!selectSuccess) {
...
app.delete(
  ...
  async function (req, res) {
    ...
    const [deleteSuccess, deleteQuery] = await SQL.query(
      "DELETE FROM `" + db.interview_locations + "` WHERE `id` = ? LIMIT 1;",
      [locationId]
    );
    if (!deleteSuccess) {
      SQL.disconnect();
...
async function getAllJobs(SQL, res) {
  const columns =
    "j.`id`, j.`hidden`, j.`title`, ja.`closed`, a.`username` as added_by, a2.`username` as edited_by";
  const queryStr =
    "SELECT " +
    columns +
    " FROM `" +
    ...
    "` j LEFT JOIN `" +
    db.job_applications +
    "` ja ON (ja.`job_id` = j.`id` AND ja.`closed` = 0) LEFT JOIN `" +
    db.admins +
    "` a ON (a.`id` = j.`added_by`) LEFT JOIN `" +
    db.admins +
    "` a2 ON (a2.`id` = j.`edited_by`) ORDER BY j.`id` DESC;";
...
app.get(
  ...
  async function (req, res) {
    ...
    const queryStr =
      "SELECT j.*, ja.`closed` FROM `" +
      db.jobs +
      "` j LEFT JOIN `" +
      db.job_applications +
      "` ja ON (ja.`job_id` = j.`id` AND ja.`closed` = 0) WHERE j.`id` = ? LIMIT 1;";
...
app.post(
  ...
  async function (req, res) {
    const jobTitle =
      req.body?.jobTitle &&
      req.body.jobTitle.trim().length > 0 &&
      req.body.jobTitle;
    ...
    const jobIcon =
      req.body?.jobIcon &&
      req.body.jobIcon.trim().length > 0 &&
      req.body.jobIcon;
    const jobResponsibilities =
      req.body?.jobResponsibilities &&
      req.body.jobResponsibilities.trim().length > 0 &&
      req.body.jobResponsibilities;
    const jobQualifications =
      req.body?.jobQualifications &&
      req.body.jobQualifications.trim().length > 0 &&
      req.body.jobQualifications;
    ...
    const [searchJobSuccess, searchJobQuery] = await SQL.query(
      "SELECT `id` FROM `" + db.jobs + "` WHERE `title` = ?",
      [jobTitle]
    );
    if (!searchJobSuccess) {
      SQL.disconnect();
    ...
    const query =
      "INSERT INTO `" +
      db.jobs +
      "` (`title`, `description`, `icon`, `responsibilities`, `qualifications`, `added_by`) VALUES (?,?,?,?,?,?)";
    const args = [
      jobTitle,
      jobDescription,
      jobIcon,
    ...
    if (!insertSuccess) {
      console.log("Failed add the job into the database");
...
async function toggleJobApply(req = app.request, res = app.response) {
  ...
  const toggleApply =
    isDigit(req.body.toggleApply) && parseInt(req.body.toggleApply);
  ...
  const [applicationSuccess, applicationQuery] = await SQL.query(
    "SELECT j.`title`, ja.`id` FROM `" +
      db.jobs +
      "` j LEFT JOIN `" +
      db.job_applications +
      "` ja ON (ja.`job_id` = j.`id` AND ja.`closed` = 0) WHERE j.`id` = ? LIMIT 1;",
  ...
  if (toggleApply === 1) {
    ...
    const [insertSuccess, insertQuery] = await SQL.query(
      "INSERT INTO `" +
        db.job_applications +
        "` (`job_id`, `job_title`) VALUES(?,?)",
      [jobId, applicationQuery[0].title]
    );
    if (!insertSuccess) {
      console.error(insertQuery);
      SQL.disconnect();
    ...
    const [updateSuccess, updateQuery] = await SQL.query(
      "UPDATE `" +
        db.job_applications +
        "` SET `closed` = 1, `close_date` = CURRENT_TIMESTAMP WHERE `id` = ?",
      [applicationQuery[0].id]
    );
    if (!updateSuccess) {
  ...
  const [logSuccess, logQuery] = await SQL.query(
    "UPDATE `" + db.jobs + "` SET `edited_by` = ? WHERE `id` = ?",
    [req.session.id, jobId]
  );
  if (!logSuccess) {
...
async function setJobVisible(req = app.request, res = app.response) {
  const jobId = parseInt(req.params.id);
  const setVisible =
    isDigit(req.body.setVisible) && parseInt(req.body.setVisible);
  ...
  const [updateSuccess, updateQuery] = await SQL.query(
    "UPDATE `" + db.jobs + "` SET `hidden` = ? WHERE `id` = ?",
    [setVisible, jobId]
  );
  if (!updateSuccess) {
    SQL.disconnect();
...
app.patch(
  ...
  async function (req, res) {
    ...
    values.push(jobId);
    const queryString =
      "UPDATE `" + db.jobs + "` SET " + updateColumnsString + " WHERE `id` = ?";
...
app.delete(
  ...
  async function (req, res) {
    ...
    const [selectSuccess, selectQuery] = await SQL.query(
      "SELECT j.`hidden`, j.`title`, ja.`id` FROM `" +
        db.jobs +
        "` j LEFT JOIN `" +
        db.job_applications +
        "` ja ON (ja.`job_id` = j.`id` AND ja.`closed` = 0) WHERE j.`id` = ? LIMIT 1;",
    ...
    if (isJobApplicationOpened) {
      ...
      res.status(403).json({
        ...
        code: 403,
        message:
          "You can't DELETE this job because it has an application opened",
      });
      return;
    ...
    console.log(
      `Job id ${jobId} title: ${selectQuery[0].title} has been deleted by: ${req.session.username}`
    );
