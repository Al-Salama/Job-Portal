...
app.post(
  ...
  async function (req, res) {
    ...
    if (!fields.jobId || !isDigit(fields.jobId)) {
      res.send(400).json({
        success: false,
        code: 400,
        message: "Bad request",
    ...
    const [searchSuccess, searchResult] = await SQL.query(
      "SELECT `jobs`.`id` as jobId, `job_applications`.`id` as appId FROM `jobs` LEFT JOIN `job_applications` ON(`job_applications`.`job_id` = `jobs`.`id` AND `job_applications`.`closed` = 0) WHERE `jobs`.`id` = ? LIMIT 1;",
      [fields.jobId]
    );
    if (!searchSuccess) {
    ...
    const [querySuccess, result] = await SQL.query(
      ...
      [
        ...
        fields.computerRate,
        fields.englishRate,
        fields.flexibilityRate,
        fields.selfTalk,
        JSON.stringify(fileExtensions, null, 0),
      ]
    );
    SQL.disconnect();
...
async function getJobs(res, SQL) {
  if (Date.now() - jobs.lastUpdate > 30_000) {
    const queryStr =
      "SELECT `jobs`.*, `job_applications`.`id` as appId, `job_applications`.`closed` FROM `jobs` LEFT JOIN `job_applications` ON(`job_applications`.`job_id` = `jobs`.`id` AND `job_applications`.`closed` = 0) WHERE `jobs`.`hidden` = 0 ORDER BY `jobs`.`id` DESC;";
